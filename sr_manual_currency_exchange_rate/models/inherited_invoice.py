    ...
    def _onchange_manual_currency_exchange_rate(self):
        if self.active_manual_currency_rate:
            ...
            for invoice in self.invoice_line_ids:
                invoice._compute_other_costs()



    @api.onchange('company_id','currency_id','partner_id')
...
class AccountMoveLine(models.Model):
   ...
   # Apply manual currency change to cost price
    @api.depends('product_id', 'move_id','move_id.manual_currency_exchange_rate')
    def _compute_other_costs(self):
        super(AccountMoveLine, self)._compute_other_costs()
        for record in self:
            if record.move_id.active_manual_currency_rate:
                # Multi-currencies.
                if record.move_id.apply_manual_currency_exchange:
                    record.cost_price = record.price_subtotal * record.move_id.manual_currency_exchange_rate