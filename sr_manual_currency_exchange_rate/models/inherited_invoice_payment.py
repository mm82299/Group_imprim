...
class AccountPayments(models.Model):
    ...
    manual_currency_exchange_rate_copy = fields.Float(string='Manual Currency Exchange Rate')
    active_manual_currency_rate = fields.Boolean('active Manual Currency', default=False)
    origina_manual_amount = fields.Float(string='Original Amount')
    ...
    def _check_manual_currency_exchange_rate(self):
        for record in self:
            if record.active_manual_currency_rate and record.manual_currency_exchange_rate == 0:
                raise ValidationError(_("Manual Currency Exchange Rate must to be different from zero ."))

    # Inherit create function to create copy of previous manual_currency_exchange_rate
    @api.model
    def create(self,vals):
        vals['manual_currency_exchange_rate_copy'] = vals['manual_currency_exchange_rate']
        res = super(AccountPayments, self).create(vals)
        return res
    # Recompute amount when change manual rate

    @api.onchange('manual_currency_exchange_rate')
    def onchange_manual_currency_exchange_rate(self):
        if self.manual_currency_exchange_rate and self.manual_currency_exchange_rate != 0:
            self.amount = self.origina_manual_amount  * self.manual_currency_exchange_rate
    ...
    def _compute_payment_difference(self):
        if self.active_manual_currency_rate:
            ...
            super(AccountPayments, self)._compute_payment_difference()
    ...
    def onchange_currency_id(self):
        if self.currency_id:
            if self.company_id.currency_id == self.currency_id:
                ...
                # self.amount *= self.manual_currency_exchange_rate * self.manual_currency_exchange_rate
            else:
    ...
    def default_get(self, fields):
        ...
        result.update({
            ...
            'manual_currency_exchange_rate_copy': move_id.manual_currency_exchange_rate,
            'origina_manual_amount' : move_id.amount_residual
